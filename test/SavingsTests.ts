...
describe("Savings Tests", () => {
  ...
  describe("Save some ofd", () => {
    ...
    it("simple save", async () => {
      await ofd.approve(savings.getAddress(), amount); // not needed if registered as minter
      await savings["save(uint192)"](amount);
    ...
    it("multi save", async () => {
      await ofd.approve(savings.getAddress(), amount * 3n);
      await savings["save(uint192)"](amount);
      await savings["save(uint192)"](amount);
      await savings["save(uint192)"](amount);
      const r = await savings.savings(owner.address);
    ...
    it("premature attempt to withdraw", async () => {
      await ofd.approve(savings.getAddress(), amount);
      await savings["save(uint192)"](amount);
    ...
    it("any interests after 365days", async () => {
      const i0 = await ofd.balanceOf(owner.address);
      ...
      /* \__ Will cause an Error, if not registered as minter. __/
        ...
        at OracleFreeDollar.permit (contracts/utils/ERC20PermitLight.sol:21)
        at OracleFreeDollar.transferFrom (contracts/utils/ERC20.sol:123)
        at Savings.refresh (contracts/Savings.sol:68)
        at Savings.withdraw (contracts/Savings.sol:109)

        The SC "Savings" is not a "minter" aka "no minter superpower". So it CAN NOT withdraw any ofd without approval,
        this will cause an error while trying to "transferFrom" the equity some interests.
      */
      const i1 = await ofd.balanceOf(owner.address);
    ...
    it("correct interest after 1000days (excl. 14days)", async () => {
      const b0 = await ofd.balanceOf(owner.address);
      const amount = floatToDec18(10_000);
      await ofd.approve(savings.getAddress(), amount);
      await savings["save(uint192)"](amount);
      const t0 = await getTimeStamp();
    ...
    it("approx. interest after 2x saves", async () => {
      const b0 = await ofd.balanceOf(owner.address);
      const amount = floatToDec18(10_000);
      await ofd.approve(savings.getAddress(), 2n * amount);
    ...
    it("withdraw savings", async () => {
      ...
      const oldReserve = await ofd.balanceOf(await ofd.reserve());
      ...
      const newReserve = await ofd.balanceOf(await ofd.reserve());
