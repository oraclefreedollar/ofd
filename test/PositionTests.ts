...
describe("Position Tests", () => {
  ...
  before(async () => {
    ...
    savings = await savingsFactory.deploy(ofd.getAddress(), 0n);
    ...
    mockXOFD = await testTokenFactory.deploy("CryptoDollar", "XOFD", 18);
    ...
    await ofd.initialize(await savings.getAddress(), "Savings");
    await ofd.initialize(await roller.getAddress(), "Roller");
    ...
    await mockXOFD.connect(owner).approve(await bridge.getAddress(), limit / 3n);
    await bridge.connect(owner).mint(limit / 3n); // owner should have plenty
    await mockXOFD.connect(bob).approve(await bridge.getAddress(), amount);
  ...
  let fLimit;
  let fGlblOFDBalanceOfCloner: bigint;
  ...
  describe("Use Minting Hub", () => {
    ...
    it("should revert creating position when minimal collateral is not worth of at least 3.5k OFD", async () => {
      await expect(
          mintingHub.openPosition(
              collateral,
              minCollateral,
    ...
    it("correct fees charged", async () => {
      ...
      let mintAfterFees =
          ...
          1000_000n;
      let cloneFeeCharged =
          fBalanceAfter - fGlblOFDBalanceOfCloner - mintAfterFees * BigInt(1e18);
  ...
  describe("challenge clone", () => {
    ...
    it("bid on challenged, expired position", async () => {
      ...
      // await mockXOFD.connect(alice).mint(alice.address, floatToDec18(bidSize));
  ...
  describe("notifying loss", async () => {
    ...
    let fFees = BigInt(fee * 1_000_000);
    let fReserve = BigInt(reserve * 1_000_000);
    let challengePeriod = BigInt(5 * 86400); // 5 days
    ...
    it("should transfer loss amount from reserve to minting hub when notify loss", async () => {
      await evm_increaseTime(86400 * 10);
  ...
  describe("position expiration auction", () => {
    ...
    it("initialize", async () => {
      ...
      await ofd.transfer(await test.getAddress(), 1000n * 10n ** 18n);
      ...
      let target = await ofd.minterReserve();
      let present = await ofd.balanceOf(await equity.getAddress());
      if (present < target){
        let amount = (target - present) / 2n;
        ofd.connect(owner).transfer(await ofd.reserve(), amount);
      }
    });

    it("should be possible to borrow after starting", async () => {
      await evm_increaseTimeTo(await pos.start());
      let balanceBefore = await ofd.balanceOf(await alice.getAddress());
      let mintedAmount = 50000n * 10n ** 18n;
      let tx = await pos.connect(alice).mint(await alice.getAddress(), mintedAmount);
      await tx.wait();
      let balanceAfter = await ofd.balanceOf(await alice.getAddress());
      expect(balanceAfter - balanceBefore).to.be.eq(39794550000000000000000n);
      expect(await pos.minted()).to.be.eq(mintedAmount);
      await ofd.transfer(await test.getAddress(), 39794550000000000000000n);
      await ofd.transfer(await test.getAddress(), 100000000000000000000000n);
    });
  ...
  describe("position rolling", () => {
    ...
    it("initialize", async () => {
      ...
      test = await factory.deploy(await mintingHub.getAddress());
      await ofd.transfer(await test.getAddress(), floatToDec18(2_000)); // opening fee
