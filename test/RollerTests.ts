...
describe("Roller Tests", () => {
  ...
  before(async () => {
    ...
    await ofd.transfer(alice.address, floatToDec18(200_000));
    await ofd.transfer(bob.address, floatToDec18(200_000));
  ...
  describe("roll tests for owner", () => {
    ...
    it("create mint and merge partially into existing position", async () => {
      await evm_increaseTime(5 * 86_400 + 300);
      const bZofd1 = await ofd.balanceOf(owner.address);
      await pos1.mint(owner.address, floatToDec18(10_000));
      const bZofd2 = await ofd.balanceOf(owner.address);
      expect(bZofd2).to.be.greaterThan(bZofd1);
  ...
  describe("roll tests for owner and alice", () => {
    ...
    it("rollFully simple", async () => {
      ...
      const b2 = await ofd.balanceOf(owner.address);
    ...
    it("rollFully check interests and rolled amount", async () => {
      ...
      const b1 = await ofd.balanceOf(owner.address);
      await ofd.transfer(bob.address, b1); // remove all ofd for testing
      ...
      const b2 = await ofd.balanceOf(owner.address);
      expect(b2).to.be.equal(0n, "owner ofd balance should be 0");
      ...
      await ofd.connect(bob).transfer(owner.address, b1); // refund ofd for testing
    });

    it("rollFully check interests and rolled amount, with 1000 ofd in wallet", async () => {
      await evm_increaseTime(5 * 86_400 + 300);
      await pos1.mint(owner.address, floatToDec18(10_000));
      const b1 = await ofd.balanceOf(owner.address);
      await ofd.transfer(bob.address, b1 - floatToDec18(1_000)); // remove all ofd for testing
      expect(await ofd.balanceOf(owner.address)).to.be.equal(
          floatToDec18(1000),
          "you should have 1000 ofd left in your wallet"
      );
